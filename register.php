
<?php

include('includes/general.php');

if ($user)
  {
    header('Location: index.php'); // Di chuyển đến file index.php
  }


if(isset($_POST['register']))
  {
    $username = @mysqli_real_escape_string($cn, $_POST['username']);
    $password = NULL;
    

    if($username==NULL)
    {
      $alert1= "Vui lòng nhập tên ";

    }

    if($_POST['password']==NULL)
    {
      $alert2= "Vui lòng nhập mật khẩu";
    }
    else
    {
      if($_POST['password'] != $_POST['re-password'])
      {
        $alert3= "Vui lòng xác minh lại mật khẩu";
      }
      else
      {
        $password = @mysqli_real_escape_string($cn, $_POST['password']);
      }
    }

    if($username && $password)
    {

      $query_check_exist_user = mysqli_query($cn, "SELECT * FROM users WHERE username = '$username'");
      if (mysqli_num_rows($query_check_exist_user))
      {
        $alert3="Đã tồn tại người dùng này, vui lòng chọn tên khác";
      }

      else
      {
                if (strlen($username) < 6 || strlen($username) > 40)
              {
                $alert1='Tên đăng nhập trong khoảng từ 6-40 ký tự.'; // Thông báo
              }
              // Nếu username chứa khoảng trắng và ký tự đặc biệt
              else if (preg_match('/\W/', $username))
              {
               $alert1= 'Tên đăng nhập không chứa khoảng trắng và ký tự đặc biệt.'; // Thông báo
              }
              // Nếu độ dài password < 6
              else if (strlen($password) < 6)
              {
                $alert2= 'Mật khẩu nên có ít nhất 6 kí tự '; // Thông báo
              }
              // Không mắc các lỗi trên thì insert vào table
              else
              {
                $password = md5($password); // Mã hoá password sang MD5
                // Thêm thông tin người dùng vào table users 
                $query_create_user = mysqli_query($cn, "INSERT INTO users VALUES (
                    '',
                    '$username',
                    '$password',
                    '$date_current' 
                  )");
                $alert= 'Đăng ký tài khoản thành công.'; // Thông báo
                
              }


      }



    }
    




  }



 ?>

<!DOCTYPE html>
<html>
<head>
	<title>Đăng ký - Gchat</title>
  <link rel="stylesheet" href="css/font-awesome-4.6.3/css/font-awesome.css">
  <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no, minimum-scale=1.0, maximum-scale=1.0"/>
	<style type="text/css">
  
		.login-page {
  width: 360px;
  padding: 8% 0 0;
  margin: auto;
}
.form {
  position: relative;
  z-index: 1;
  background: #FFFFFF;
  max-width: 360px;
  margin: 0 auto 100px;
  padding: 45px;
  text-align: center;
  box-shadow: 0 0 20px 0 rgba(0, 0, 0, 0.2), 0 5px 5px 0 rgba(0, 0, 0, 0.24);
}
.form input {
  font-family: "Roboto", sans-serif;
  outline: 0;
  background: #cccccc;
  width: 100%;
  border: 0;
  margin: 0 0 15px;
  padding: 15px;
  box-sizing: border-box;
  font-size: 14px;
}
.form button {
  font-family: "Roboto", sans-serif;
  text-transform: uppercase;
  outline: 0;
  background: #666666;
  width: 100%;
  border: 0;
  padding: 15px;
  color: #FFFFFF;
  font-size: 14px;
  -webkit-transition: all 0.3 ease;
  transition: all 0.3 ease;
  cursor: pointer;
}
.form button:hover,.form button:active,.form button:focus {
  background: #4d4d4d;
}
.form .message {
  margin: 15px 0 0;
  color: #737373;
  font-size: 12px;
}
.form .message a {
  color: #000000;
  text-decoration: none;
}
.form .register-form {
  display: none;
}

.form .alert {
  color: #ff0000;
}

.form .success {
  color :#33cc33;
}
.container {
  position: relative;
  z-index: 1;
  max-width: 300px;
  margin: 0 auto;
}
.container:before, .container:after {
  content: "";
  display: block;
  clear: both;
}
.container .info {
  margin: 50px auto;
  text-align: center;
}
.container .info h1 {
  margin: 0 0 15px;
  padding: 0;
  font-size: 36px;
  font-weight: 300;
  color: #1a1a1a;
}
.container .info span {
  color: #4d4d4d;
  font-size: 12px;
}
.container .info span a {
  color: #000000;
  text-decoration: none;
}
.container .info span .fa {
  color: #EF3B3A;
}
body {
  background: #737373; /* fallback for old browsers */
  background: -webkit-linear-gradient(right, #737373, #4d4d4d);
  background: -moz-linear-gradient(right, #737373, #4d4d4d);
  background: -o-linear-gradient(right, #737373, #4d4d4d);
  background: linear-gradient(to left, #737373, #4d4d4d);
  font-family: "Roboto", sans-serif;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;    
}
	</style>
	<script type="text/javascript" src="/jquery.min.js">
		$('.message a').click(function(){
   $('form').animate({height: "toggle", opacity: "toggle"}, "slow");
});

    




	</script>
</head>
<body>
<div class="login-page">
  <div class="form">
    <h1><i class="fa fa-commenting"></i> GChat</h1>
    <form class="login-form" action="register.php" method="POST">
      <input type="text" placeholder="Tên đăng nhập" name="username" id="ten"/>
      <div class="alert"><?php if(@$alert1){ echo $alert1 ;}?></div>
      <div id="alert"></div>
      <input type="password" placeholder="Mật khẩu" name="password"/>
      <div class="alert"><?php if(@$alert2){ echo $alert2 ;}?></div>
      <input type="password" placeholder="Xác minh mật khẩu" name="re-password"/>
      <div class="alert"><?php if(@$alert3){ echo $alert3 ;}?></div>
      
      <button type="submit" name="register">Đăng ký</button>
      <div class="success"><?php if(@$alert){ echo $alert ;}?></div>
      <p class="message">Đã có tài khoản? <a href="login.php">Đăng nhập</a></p>
    </form>
  </div>
</div>
</body>
</html>